...
public class CatalogosService : ICatalogosService
{
    ...
    private readonly NeoSTP.Application.Lookups.ILookupCacheInvalidator? _lookupInvalidator;

    public CatalogosService(NeoStpDbContext db, IAuditoriaService auditoria,
        NeoSTP.Application.Lookups.ILookupCacheInvalidator? lookupInvalidator = null)
    {
        _db = db;
        _auditoria = auditoria;
        _lookupInvalidator = lookupInvalidator;
    }

    private Task InvalidarLookupsAsync(CancellationToken ct)
        => _lookupInvalidator?.InvalidarCatalogosAsync(ct) ?? Task.CompletedTask;
