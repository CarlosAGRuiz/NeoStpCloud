...
- Pruebas locales: 2,442 unitarias y 9 de integración aprobadas; los gates adicionales de SQL Server real también fueron aprobados.
...
- [x] Revisar el SQL generado y ensayarlo en `NeoSTP_Staging`; la prueba real detectó y corrigió opciones de sesión, cobertura incompleta del artefacto y backfill de columnas nuevas.
...
## Sprint actual: Semántica de precio e IVA

- [x] Definir `IVA_INCLUIDO` y `IVA_EXCLUIDO` por producto con compatibilidad `IVA_INCLUIDO` para datos existentes.
- [x] Centralizar cálculo, redondeo, descuentos y conversión 113 ↔ 100 + 13.
- [x] Persistir snapshots de semántica en líneas POS y cotizaciones para que cambios futuros del producto no alteren históricos.
- [x] Normalizar promociones POS/Cotización hacia Factura 01 o CCF 03 sin inflar ni reducir el total comercial.
- [x] Conservar el contrato histórico del endpoint DTE cuando `TipoPrecio` no se informa.
...
- [x] Crear `NeoSTP_Staging` separada y aplicar las 94 migrations del repositorio.
