...
$previous=Get-Content -LiteralPath (Join-Path $repo 'tmp/neo-production/logs/attempt2/clone-manifest.json') -Raw|ConvertFrom-Json
if(-not $previous.Verified -or -not $previous.BackupCompleted -or $previous.TargetDatabase -notmatch '^NeoProductionAudit_[a-f0-9]{32}$'){throw 'Verified backup identity missing.'}
$expectedBackup=Join-Path (Join-Path $allowedBackupRoot $previous.TargetDatabase) 'source-copy-only.bak'
if([IO.Path]::GetFullPath($previous.BackupFile)-ne $expectedBackup -or (Get-FileHash -LiteralPath $expectedBackup -Algorithm SHA256).Hash-ne $previous.BackupSha256){throw 'Verified backup path or checksum changed.'}
...
'@)
 ...
 $manifest.ReusedVerifiedBackup=$true;$manifest.PreviousTargetDatabase=$previous.TargetDatabase
 $manifest.BackupCompleted=$true;Write-Output 'Existing verified COPY_ONLY backup reused; no source backup performed.'
